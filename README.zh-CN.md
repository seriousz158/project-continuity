...
**版本：** `0.1.0`。该发布线在 macOS、Linux、Windows 及 Python 3.11、3.12、
3.13 上测试；只有完整九任务矩阵通过的提交才会创建稳定 tag。当前不声称既有项目
已经迁移，也不把磁盘安装等同于活动客户端已热加载。

## 验证范围

- 源码目录和干净解压的发行包运行同一组 52 项测试。
- GitHub Actions 在九种 OS/Python 组合中运行该测试组与打包校验。
- 一个私有、隔离的复杂项目演练完成了权威读取方切换；迁移新增 verifier 失败为 0，
  原有业务失败原样保留，并且没有授权真实项目切换。
- 客户端发现、磁盘安装、活动会话加载以及每个既有项目的迁移均须分别验证，不能由
  CI 结果推断。
