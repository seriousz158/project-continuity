...
class SecurityRegressions(unittest.TestCase):
    ...
    def test_benign_task_identifier_is_not_a_credential(self):
        ...
        self.current.write_text(doc, encoding="utf-8")
    ...
    def test_separated_secret_prefix_and_escaped_sk_are_rejected(self):
        ...
        self.current.write_text(doc.replace(fake, r"\u0073k-" + "A" * 24), encoding="utf-8")
    ...
    def test_decoded_state_sensitive_values_are_rejected_before_status_output(self):
        ...
        self.current.write_text(doc.replace(fake, r"\u0067hp_" + "A" * 36), encoding="utf-8")
    ...
    def test_decoded_extensions_are_also_scanned(self):
        ...
        self.current.write_text(doc.replace(fake, r"\u0067hp_" + "B" * 36), encoding="utf-8")
    ...
    def test_decoded_git_baseline_is_scanned(self):
        fake = "ghp_" + "C" * 36
        lines, body, _ = cli.split(self.current.read_text(encoding="utf-8"))
        raw = json.dumps({"kind": "none", "extra": fake}).replace(fake, r"\u0067hp_" + "C" * 36)
        self.current.write_text(cli.metadata(lines, {"git_baseline": raw}) + body, encoding="utf-8")
        code, out, err = self.call("status")
        self.assertEqual(code, 2)
        self.assertNotIn(fake, out + err)
    ...
    def test_snapshot_recovery_preserves_current_extension_overlay(self):
        ...
        self.current.write_text(self.state_document(lambda s: s["extensions"].update({"keep": {"nested": "current"}})), encoding="utf-8")
    ...
    def test_malformed_reserved_recovery_log_fails_closed(self):
        self.current.write_text(self.state_document(lambda s: s["extensions"].update({"recovery_log": "not a list"})), encoding="utf-8")
        before = self.current.read_bytes()
